    ...
    while not stop_event.is_set():
        try:
            ...
            advice_response = agent_executor.invoke({
                "input": f"Current price of {crypto_symbol} is {current_price}. Here is the trade log: {trade_log_str}. Should I buy, sell, or hold?"
            })
...
chat_model = ChatBedrock(model_id=chat_model_id).bind_tools([fetch_current_price, fetch_historical_prices, simulate_crypto_trading])
...
tools = [fetch_current_price, fetch_historical_prices, simulate_crypto_trading]
